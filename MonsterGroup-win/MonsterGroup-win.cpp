...
#include <chrono>
#include <ctime>
#include <windows.h> // For memory information
#include <psapi.h> // For process memory information

#include <iomanip> // For setw and setfill
//const size_t N = 18317; // 10 GiB
//const size_t N = 196883;
const size_t N = 1000; // 10 GiB
...
double getSystemMemory()
{
    ...
    return memInfo.ullTotalPhys / double(1024 * 1024 * 1024);
}
...
double getProcessMemory()
{
    ...
    return virtualMemUsedByMe / double(1024 * 1024 * 1024);
}
...
void printExecutionTime(std::chrono::system_clock::time_point& start, const std::string& message)
{
    ...
    std::cout << seconds << "s ";
    std::cout << std::fixed << std::setprecision(0) << milliseconds << "ms ";
...
void multiplyMatrices(std::vector<std::vector<std::complex<double>>>& matrixA, std::vector<std::vector<std::complex<double>>>& matrixB,
    size_t startRow, size_t endRow, std::atomic<int>& complexMultiplications)
{
...
int main()
{
    ...
    double totalSystemMemGiB = getSystemMemory();
    // Create a std::atomic variable for the thread-safe counter
    std::atomic<int> complexMultiplications(0);
    ...
    for (size_t multiplicationStep = 0; multiplicationStep < multiplicationStepCount; ++multiplicationStep)
    {
        ...
        for (size_t i = 0; i < numThreads; ++i)
        {
            size_t startRow = i * rowsPerThread;
            size_t endRow = (i == (numThreads - 1)) ? N : (startRow + rowsPerThread);
            threads[i] = std::thread(multiplyMatrices, std::ref(matrixA), std::ref(matrixB), startRow, endRow, std::ref(complexMultiplications));
        }
        for (auto& th : threads) th.join(); // Ensure all threads finish
        // Print the total complex multiplications after each loop iteration
        std::cout << "Total complex multiplications after loop " << multiplicationStep + 1 << ": " << complexMultiplications << std::endl;
